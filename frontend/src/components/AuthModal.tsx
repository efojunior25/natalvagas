  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        if (accountType === 'COMPANY' && (!companyName.trim() || cnpj.replace(/\D/g, '').length !== 14)) {
          setErrorMsg('Informe a razão social e um CNPJ válido.');
          setLoading(false);
          return;
        }
        const res = await registerWithEmail(name, email, password, { accountType, companyName, cnpj });
        if (res.success) {
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          ...
          {requiresMfaSetup && (
            <div className="space-y-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>Confirmar acesso EDITDEV</span>}
                </button>
            ...
            <div className="space-y-4">
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <button
                  ...
                  className="w-full py-3 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-bold text-sm shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                >
                  {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>Entrar no modo EDITDEV</span>}
                </button>
          ...
          {!requiresMfa && !requiresMfaSetup && (
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                {isRegister && (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button type="button" onClick={() => setAccountType('USER')} className={`py-2 rounded-xl text-xs font-bold border ${accountType === 'USER' ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-slate-600 border-slate-200'}`}>Sou candidato</button>
                    <button type="button" onClick={() => setAccountType('COMPANY')} className={`py-2 rounded-xl text-xs font-bold border ${accountType === 'COMPANY' ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-slate-600 border-slate-200'}`}>Sou empresa</button>
                  </div>
                  <div>
                  ...
                  {accountType === 'COMPANY' && (
                    <div className="grid grid-cols-1 gap-3">
                      <div><label className="text-xs font-semibold text-slate-700 block mb-1">Razão social</label><input type="text" required value={companyName} onChange={(e) => setCompanyName(e.target.value)} className="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 focus:border-brand-500 focus:outline-hidden" /></div>
                      <div><label className="text-xs font-semibold text-slate-700 block mb-1">CNPJ</label><input type="text" inputMode="numeric" required value={cnpj} onChange={(e) => setCnpj(e.target.value)} placeholder="00.000.000/0000-00" className="w-full px-3 py-2.5 text-sm rounded-xl border border-slate-200 focus:border-brand-500 focus:outline-hidden" /></div>
                    </div>
                  )}
                  </>
                )}
              ...
              {!isRegister && <div className="text-center"><a href="/recuperar-senha" className="text-xs text-brand-600 font-bold hover:underline">Esqueci minha senha</a></div>}
            </>
          )}
