              ...
              {isAuthenticated ? (
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 border border-slate-200/50 dark:border-slate-700">
                    ...
                    {user?.role === 'EDITDEV' ? (
                      <span className="bg-emerald-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md flex items-center gap-1">
                  ...
                  {user?.role === 'EDITDEV' && (
                    <Link
                      to="/editdev"
                      className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-black text-amber-400 text-xs font-bold transition-all border border-amber-500/40 shadow-xs"
                      title="Abrir Painel de Gestão de Vagas"
                    >
                      <ShieldCheck className="w-3.5 h-3.5 text-amber-400" />
                      <span className="hidden lg:inline">Painel EDITDEV</span>
                    </Link>
                  )}
                  <button
                    type="button"
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {user?.role === 'EDITDEV' ? (
                          <span className="bg-emerald-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-sm flex items-center gap-0.5">
                            <ShieldCheck className="w-2.5 h-2.5" />
                            EDITDEV
                          </span>
                        ) : user?.isPro ? (
                      ...
                      {user?.role === 'EDITDEV' && (
                        <Link
                          to="/editdev"
                          onClick={() => setIsMobileMenuOpen(false)}
                          className="inline-flex items-center gap-1 text-[11px] font-extrabold text-amber-600 dark:text-amber-400 mt-1 hover:underline"
                        >
                          <ShieldCheck className="w-3 h-3" />
                          <span>Abrir Painel EDITDEV</span>
                        </Link>
                      )}
                    </div>
                  </div>
