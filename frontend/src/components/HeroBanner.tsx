    ...
    <section className="relative overflow-hidden bg-gradient-to-b from-brand-50/80 via-white to-surface-lightBg py-12 md:py-16 border-b border-slate-200">
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          ...
          <button 
            type="submit"
            className="flex items-center justify-center gap-2 px-6 py-3 bg-brand-600 hover:bg-brand-700 active:scale-98 text-white font-semibold text-sm rounded-xl shadow-md shadow-brand-500/25 transition-all"
          >
        ...
        <div className="mt-4 flex items-center justify-center gap-2 text-xs text-slate-500 flex-wrap">
          <span className="font-medium text-slate-600">Mais buscados:</span>
          {['Auxiliar Administrativo', 'Atendente', 'Vendedor Alecrim', 'Jovem Aprendiz', 'Ponta Negra'].map((term) => (
            <button
              key={term}
              type="button"
              onClick={() => {
              ...
              className="px-2.5 py-1 rounded-md bg-slate-100 hover:bg-brand-50 hover:text-brand-700 transition-colors"
            >
              {term}
            </button>
          ))}
